...
select c.change_object,	c.changed_item_id, c.change_user
	into @changeObject, @eventId, @user
from changes c
...
if @newVenueJSON is not null then
	...
	values (
		...
		json_unquote(json_extract(@newVenueJSON, "$.address2")),
		...
		case @operation
			...
			when "add" then
				begin
					...
					values (
						@eventId,
						json_unquote(json_extract(@daysJSON, concat("$[",@x,"].value.datetime"))),
						json_unquote(json_extract(@daysJSON, concat("$[",@x,"].value.doors"))),
						json_unquote(json_extract(@daysJSON, concat("$[",@x,"].value.description")))
					);
				end;
